...
def standardize_species_name_for_id(original_name):
    """
    Convert nama spesies menjadi format Genus_spesies atau Genus.
    Dipakai di ID SeqRecord, nama daun di pohon, dan dasar nama file gambar.
    """
...
def sanitize_filename_for_display(original_name_for_file):
    """
    Membersihkan nama spesies asli (dengan penulis/tahun)
    """
...
def generate_dummy_newick_for_subset(species_names_std_subset):
    ...
    if not unique_ordered_subset: 
        return None

    if len(unique_ordered_subset) == 1: 
        return f"({unique_ordered_subset[0]}:0.1);"
...
def main():
    ...
    for target_species_std in list_of_target_species_std_keys: 
        ...
        if not subset_species_std_for_dummy : 
            subset_species_std_for_dummy = [target_species_std] if target_species_std else []
